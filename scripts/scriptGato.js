...
let containerGato = document.getElementById("containerGato")
...
//Mostrar productos por categoría (gato)
document.addEventListener("DOMContentLoaded", async () => {
    let res = await fetch(" http://localhost:4000/Productos")
    let data = await res.json()
    // console.log(data)

    let categoriaGato = data.filter(item => item.categorie == "Gatos")
    showProducto(categoriaGato, containerGato)
})
